...
- (id)initWithPrimaryKey:(NSInteger)pk database:(sqlite3 *)db {
    if (self = [super init]) {
        ...
        if (init_statement == nil) {
            const char *sql = "SELECT name, type, weight, data_set FROM data_panels WHERE pk=?";
		...
		if (sqlite3_step(init_statement) == SQLITE_ROW) {
			...
			self.dataSet = [[DataSet alloc] initWithPrimaryKey:sqlite3_column_int(init_statement, 3) database:database];
        } else {
            self.name = @"No name";
        }
